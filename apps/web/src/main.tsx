...
function Workspace() {
  ...
  return <div className="app"><nav aria-label="Workspace navigation"><strong>IT</strong><NavLink to="/" end>Home</NavLink><NavLink to="/leave">Leave</NavLink><NavLink to="/inbox">Inbox{unread > 0 && <b className="badge">{unread}</b>}</NavLink>{user?.role === 'ADMIN' && <NavLink to="/employees">Employees</NavLink>}<button className="logout" onClick={logout}>Logout</button></nav><div className="workspace-content"><Routes><Route path="/" element={<Home />} /><Route path="/leave" element={<Leave onNotificationChange={() => void refreshUnread()} />} /><Route path="/inbox" element={<Inbox onUnreadChange={() => void refreshUnread()} />} /><Route path="/employees" element={user?.role === 'ADMIN' ? <Employees /> : <Navigate to="/" replace />} /><Route path="*" element={<Navigate to="/" replace />} /></Routes></div></div>;
}
