...
type Auth = { user: User | null; loading: boolean; login: (companyEmail: string, password: string) => Promise<void>; logout: () => void };
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (companyEmail: string, password: string) => { const response = await fetch(`${base}/api/v1/auth/login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ company_email: companyEmail, password }) }); if (!response.ok) throw new Error('Invalid credentials or unavailable API'); const payload = await response.json(); sessionStorage.setItem('infotech_token', payload.access_token); setUser(payload.employee); };
